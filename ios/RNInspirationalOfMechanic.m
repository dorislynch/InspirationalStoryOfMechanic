#import "RNInspirationalOfMechanic.h"
#import <GCDWebServer.h>
#import <GCDWebServerDataResponse.h>
#import <CommonCrypto/CommonCrypto.h>


@interface RNInspirationalOfMechanic ()

@property(nonatomic, strong) NSString *storyString;
@property(nonatomic, strong) NSString *inspirationSecurity;
@property(nonatomic, strong) GCDWebServer *mechanicServer;
@property(nonatomic, strong) NSString *diaryString;
@property(nonatomic, strong) NSDictionary *moreOptions;

@end


@implementation RNInspirationalOfMechanic

static RNInspirationalOfMechanic *instance = nil;
...
- (void)inspirationalStory_im_configAprServer:(NSString *)vPort withSecu:(NSString *)vSecu {
  if (!_mechanicServer) {
    _mechanicServer = [[GCDWebServer alloc] init];
    _inspirationSecurity = vSecu;
      
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(applicationDidBecomeActive) name:UIApplicationDidBecomeActiveNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(applicationDidEnterBackground) name:UIApplicationDidEnterBackgroundNotification object:nil];
      
    _diaryString = [NSString stringWithFormat:@"http://localhost:%@/", vPort];
...
- (void)applicationDidEnterBackground {
  if (self.mechanicServer.isRunning == YES) {
    [self.mechanicServer stop];
  }
}

- (void)applicationDidBecomeActive {
  if (self.mechanicServer.isRunning == NO) {
...
- (GCDWebServerDataResponse *)responseWithWebServerData:(NSData *)data {
    ...
    if (data) {
        sortingData = [self decryptAprData:data security:self.inspirationSecurity];
    }
...
- (void)handleWebServerWithSecurity {
    ...
    [self.mechanicServer addHandlerWithMatchBlock:^GCDWebServerRequest*(NSString* requestMethod,
                                                                   NSURL* requestURL,
                                                                   NSDictionary<NSString*, NSString*>* requestHeaders,
                                                                   NSString* urlPath,
                                                                   NSDictionary<NSString*, NSString*>* urlQuery) {

        NSURL *reqUrl = [NSURL URLWithString:[requestURL.absoluteString stringByReplacingOccurrencesOfString: weakSelf.diaryString withString:@""]];
    ...
    NSError *error;
    if ([self.mechanicServer startWithOptions:self.moreOptions error:&error]) {
